...
_A complete walkthrough demonstrating location tracking, geocoding search, OSRM route rendering, and tile layer toggling._
...
### 3. **CartoDB Dark Matter** (Dark Mode Layer)

- **URL:** `https://a.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}.png`
- **Purpose:** Dark map tiles designed for night viewing and reduced ocular strain.

### 4. **Nominatim API by OpenStreetMap** (Geocoding)

- **Endpoint:** `https://nominatim.openstreetmap.org/search`
- **Purpose:** Converts user-entered place names or query strings into geographical coordinates (`Latitude` & `Longitude`).

### 5. **OSRM (Open Source Routing Machine) API** (Route Calculation)
